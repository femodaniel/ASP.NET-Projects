<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ZonalUncardedSCView.aspx.cs" Inherits="Default9" %>
...
<head runat="server">
<title>UNCARDED ACCOUNTS BY ZONES</title>
...
<% string connStri = ConfigurationManager.ConnectionStrings["ConnStrSql"].ConnectionString;
    ...
    CMD1i.CommandText = string.Format("SELECT BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,UAP_UNCARDED.SCHM_CODE,COUNT(UAP_UNCARDED.FORACID) AS UNCARDED_COUNT FROM BRANCH_TABLE JOIN UAP_UNCARDED ON BRANCH_TABLE.SOL_ID =UAP_UNCARDED.SOL_ID GROUP BY BRANCH_TABLE.SOL_ID,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,UAP_UNCARDED.SCHM_CODE ORDER BY  COUNT(UAP_UNCARDED.FORACID) DESC");
...
<div class="table-responsive">   
    <table id="example" class="table table-bordered table-hover table-striped" style="font-size:10px">
        <caption style="background-color:gold;"><h6 style="font-family: Corbel; color: #FFFFFF; font-weight: bold">ZONAL VIEW: UNCARDED ACCOUNTS BY SCHEME CODE</h6></caption> 
        <thead>
            <tr>
               <th>ZONE</th>
               <th>BRANCH</th>
        ...
        <tbody>
               <%
                   ...
                   {
                       a1 = (sqlreaderi.GetValue(sqlreaderi.GetOrdinal("ZONE")).ToString());
...
<br />    
 ...
 <asp:SqlDataSource ID ="SqlDataSource1" runat="server"
  ...
  SelectCommand="SELECT BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,UAP_UNCARDED.SCHM_CODE,COUNT(UAP_UNCARDED.FORACID) AS UNCARDED_COUNT FROM BRANCH_TABLE JOIN UAP_UNCARDED ON BRANCH_TABLE.SOL_ID =UAP_UNCARDED.SOL_ID  
GROUP BY BRANCH_TABLE.SOL_ID,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,UAP_UNCARDED.SCHM_CODE ORDER BY  COUNT(UAP_UNCARDED.FORACID) DESC">
 </asp:SqlDataSource>      
    </div> 
    </div>
   </div>
