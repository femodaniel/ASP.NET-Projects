<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RegionCardableSCView.aspx.cs" Inherits="Default2" %>
...
<title>REGION VIEW:CARDABLE ACCOUNTS </title>
<meta charset="utf-8"/>
...
<% string connStri = ConfigurationManager.ConnectionStrings["ConnStrSql"].ConnectionString;
    ...
    CMD1i.CommandText = string.Format("SELECT BRANCH_TABLE.REGION,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,UAP_CARDABLE.SCHM_CODE,COUNT(UAP_CARDABLE.FORACID) AS CARDABLE_COUNT FROM BRANCH_TABLE JOIN UAP_CARDABLE ON BRANCH_TABLE.SOL_ID =UAP_CARDABLE.SOL_ID GROUP BY BRANCH_TABLE.SOL_ID,BRANCH_TABLE.REGION,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,UAP_CARDABLE.SCHM_CODE ORDER BY  COUNT(UAP_CARDABLE.FORACID) DESC");
...
<div class="table-responsive">   
    <table id="example" class="table table-bordered table-hover table-striped" style="font-size:10px">
        <caption style="background-color:gold;"><h6 style="font-family: Corbel; color: #FFFFFF; font-weight: bold">REGION VIEW: CARDABLE ACCOUNTS BY SCHEME CODE</h6></caption> 
        <thead>
            <tr>
                <th>REGION</th>
                   <th>ZONE</th>
        ...
        <tbody>
               <%
                   ...
                   {
                        a1 = (sqlreaderi.GetValue(sqlreaderi.GetOrdinal("REGION")).ToString());
                        a2 = (sqlreaderi.GetValue(sqlreaderi.GetOrdinal("ZONE")).ToString());
                        a3 = (sqlreaderi.GetValue(sqlreaderi.GetOrdinal("BRANCH")).ToString());
                        a4 = (sqlreaderi.GetValue(sqlreaderi.GetOrdinal("SCHM_CODE")).ToString());
                        a5 = Convert.ToDouble(sqlreaderi.GetValue(sqlreaderi.GetOrdinal("CARDABLE_COUNT")).ToString());                   
              

                   %>
              <tr>
                  <td><% Response.Write(a1); %></td>
                  <td><% Response.Write(a2); %></td>
                  <td><% Response.Write(a3); %></td>
                  <td><% Response.Write(a4); %></td>
