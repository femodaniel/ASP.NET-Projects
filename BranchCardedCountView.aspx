<%@ Page Language="C#" AutoEventWireup="true" CodeFile="BranchCardedCountView.aspx.cs" Inherits="Default7" %>
...
<head runat="server">
   <title>CARDED ACCOUNTS BY BRANCHES</title>
<meta charset="utf-8"/>
...
<% string connStri = ConfigurationManager.ConnectionStrings["ConnStrSql"].ConnectionString;
    ...
    CMD1i.CommandText = string.Format("SELECT BRANCH_TABLE.REGION ,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,COUNT(UAP_CARDED.ACCOUNT_ID) AS CARDCOUNT FROM BRANCH_TABLE JOIN UAP_CARDED ON BRANCH_TABLE.SOL_ID =UAP_CARDED.BRANCH_CODE GROUP BY BRANCH_TABLE.SOL_ID,BRANCH_TABLE.REGION,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH ORDER BY  COUNT(UAP_CARDED.ACCOUNT_ID) DESC");
...
<div class="table-responsive">   
    <table id="example" class="table table-bordered table-hover table-striped" style="font-size:10px">
        <caption style="background-color:gold;"><h6 style="font-family: Corbel; color: #FFFFFF; font-weight: bold">BRANCH CARDED ACCOUNTS</h6></caption> 
        <thead>
            <tr>
                <th>REGION</th>
                <th>ZONE</th>
        ...
        <tbody>
               <%
                   ...
                   while (sqlreaderi.Read())
                   {
                       ...
                       a5 = Convert.ToDouble(sqlreaderi.GetValue(sqlreaderi.GetOrdinal("CARDCOUNT")).ToString());                      
...
<br />    
 ...
 <asp:SqlDataSource ID ="SqlDataSource1" runat="server"
  ...
  SelectCommand="SELECT BRANCH_TABLE.REGION ,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH,COUNT(UAP_CARDED.ACCOUNT_ID) AS CARDCOUNT FROM BRANCH_TABLE JOIN UAP_CARDED ON BRANCH_TABLE.SOL_ID =UAP_CARDED.BRANCH_CODE 
GROUP BY BRANCH_TABLE.SOL_ID,BRANCH_TABLE.REGION,BRANCH_TABLE.ZONE,BRANCH_TABLE.BRANCH ORDER BY  COUNT(UAP_CARDED.ACCOUNT_ID) DESC">
 </asp:SqlDataSource>      
