<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ZonalCardableCountView.aspx.cs" Inherits="Default3" %>
...
<head runat="server">
<title> Zonal View: Cardable Count</title>
<meta charset="utf-8"/>
...
<% string connStri = ConfigurationManager.ConnectionStrings["ConnStrSql"].ConnectionString;
    ...
    CMD1i.CommandText = string.Format("SELECT BRANCH_TABLE.ZONE,COUNT(UAP_CARDABLE.FORACID) AS CARDABLE_COUNT FROM BRANCH_TABLE JOIN UAP_CARDABLE ON BRANCH_TABLE.SOL_ID =UAP_CARDABLE.SOL_ID GROUP BY BRANCH_TABLE.ZONE ORDER BY  COUNT(UAP_CARDABLE.FORACID) DESC");
...
<div class="table-responsive">   
    <table id="example" class="table table-bordered table-hover table-striped" style="font-size:10px">
        <caption style="background-color:gold;"><h6 style="font-family: Corbel; color: #FFFFFF; font-weight: bold">ZONAL VIEW: CARDABLE ACCOUNTS</h6></caption> 
        <thead>
            <tr>
                <th>ZONE</th>
...
<br />    
 ...
 <asp:SqlDataSource ID ="SqlDataSource1" runat="server"
  ...
  SelectCommand="SELECT BRANCH_TABLE.ZONE,COUNT(UAP_CARDABLE.FORACID) AS CARDABLE_COUNT FROM BRANCH_TABLE JOIN UAP_CARDABLE ON BRANCH_TABLE.SOL_ID =UAP_CARDABLE.SOL_ID  
  GROUP BY BRANCH_TABLE.ZONE ORDER BY  COUNT(UAP_CARDABLE.FORACID) DESC">
 </asp:SqlDataSource>      
    </div> 
    </div>
   </div>
