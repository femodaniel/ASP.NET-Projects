<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ZonalUncardedCountView.aspx.cs" Inherits="Default3" %>
...
<head runat="server">
<title>Zonal View: UnCarded Count</title>
...
<% string connStri = ConfigurationManager.ConnectionStrings["ConnStrSql"].ConnectionString;
    ...
    CMD1i.CommandText = string.Format("SELECT BRANCH_TABLE.ZONE,COUNT(UAP_UNCARDED.FORACID) AS UNCARDED_COUNT FROM BRANCH_TABLE JOIN UAP_UNCARDED ON BRANCH_TABLE.SOL_ID =UAP_UNCARDED.SOL_ID GROUP BY BRANCH_TABLE.ZONE ORDER BY  COUNT(UAP_UNCARDED.FORACID) DESC");
...
<div class="table-responsive">   
    <table id="example" class="table table-bordered table-hover table-striped" style="font-size:10px">
        <caption style="background-color:gold;"><h6 style="font-family: Corbel; color: #FFFFFF; font-weight: bold">ZONAL VIEW: UNCARDED ACCOUNTS COUNT</h6></caption> 
        <thead>
            <tr>
                <th>ZONE</th>
        ...
        <tbody>
               <%
                   ...
                   while (sqlreaderi.Read())
                   {
                       ...
                       a3 = Convert.ToDouble(sqlreaderi.GetValue(sqlreaderi.GetOrdinal("UNCARDED_COUNT")).ToString());                      
...
<br />    
 ...
 <asp:SqlDataSource ID ="SqlDataSource1" runat="server"
  ...
  SelectCommand="SELECT BRANCH_TABLE.ZONE,COUNT(UAP_UNCARDED.FORACID) AS UNCARDED_COUNT FROM BRANCH_TABLE JOIN UAP_UNCARDED ON BRANCH_TABLE.SOL_ID =UAP_UNCARDED.SOL_ID  
  GROUP BY BRANCH_TABLE.ZONE ORDER BY  COUNT(UAP_UNCARDED.FORACID) DESC">
 </asp:SqlDataSource> 
